...
exports.registerFraudAndCredentialTools = registerFraudAndCredentialTools;
const zod_1 = require("zod");
const apiClient_js_1 = require("../services/apiClient.js");
const constants_js_1 = require("../constants.js");
function registerFraudAndCredentialTools(server) {
    // ── Tool 1: Query Fraud Registry ──────────────────────────────────────────
    server.registerTool("fraud_query_registry", {
...
Critical: BLOCK on known_bad_actor=true or risk_score > 60. Escalate immediately.`,
        inputSchema: zod_1.z.object({
            entity_type: zod_1.z.enum(["pan", "gstin", "aadhaar", "phone", "account", "invoice"])
                .describe("Type of entity to check"),
            entity_value: zod_1.z.string().min(1).describe("Entity value to look up in fraud registry"),
            case_id: zod_1.z.string().optional().describe("Current loan case ID for audit logging"),
        }),
        ...
        const result = await (0, apiClient_js_1.callApi)(mockKey, async () => {
            ...
            const resp = await axios.post(constants_js_1.API_ENDPOINTS.FRAUD_REGISTRY, { entity_type, entity_value, case_id }, { headers: { "X-API-Key": constants_js_1.API_KEYS.FRAUD_API_KEY } });
            return resp.data;
        });
        const verdict = result.known_bad_actor || result.risk_score > constants_js_1.THRESHOLDS.MAX_FRAUD_RISK_SCORE
            ? "BLOCK"
        ...
        const output = {
            ...result,
            risk_level: (0, apiClient_js_1.riskLevel)(result.risk_score),
    ...
    server.registerTool("fraud_submit_signal", {
...
Returns: { "signal_id": string, "accepted": boolean, "message": string }`,
        inputSchema: zod_1.z.object({
            entity_type: zod_1.z.enum(["pan", "gstin", "aadhaar", "phone", "account", "invoice"]),
            entity_value: zod_1.z.string().min(1),
            signal_type: zod_1.z.string().describe("Signal category, e.g. TAMPERED_DOCUMENT, FAKE_INVOICE, IDENTITY_LAYERING"),
            severity: zod_1.z.enum(["low", "medium", "high", "critical"]),
            description: zod_1.z.string().min(10).describe("Clear description of the fraud signal discovered"),
            case_id: zod_1.z.string().describe("Loan case ID where this was found"),
        }),
        ...
        const output = {
            ...
            submitted_at: (0, apiClient_js_1.now)(),
        };
        return {
            content: [{ type: "text", text: JSON.stringify(output, null, 2) }],
            structuredContent: output,
    ...
    server.registerTool("credential_verify_dental", {
...
This check is unique to dental/medical professional loans.`,
        inputSchema: zod_1.z.object({
            registration_number: zod_1.z.string().describe("DCI registration number from degree certificate"),
            dentist_name: zod_1.z.string().describe("Full name as on the loan application"),
            degree_type: zod_1.z.enum(["BDS", "MDS", "BOTH"]).describe("Type of dental degree to verify"),
        }),
        ...
        const result = await (0, apiClient_js_1.callApi)(mockKey, async () => {
            const axios = (await import("axios")).default;
            const resp = await axios.get(`${constants_js_1.API_ENDPOINTS.DENTAL_COUNCIL}/verify`, {
                params: { registration_number, degree_type },
                headers: { "X-API-Key": constants_js_1.API_KEYS.DENTAL_API_KEY }
            });
            return resp.data;
        });
        const reasons = [...result.risk_flags];
        if (!result.credential_verified)
            reasons.push("Credential could not be verified with DCI");
        if (result.registration_status === "revoked")
            reasons.push("CRITICAL: Registration revoked");
        if (result.registration_status === "not_found")
            reasons.push("Registration number not found in DCI database");
...
//# sourceMappingURL=fraudAndCredentialTools.js.map