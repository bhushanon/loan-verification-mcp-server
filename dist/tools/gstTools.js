...
exports.registerGstTools = registerGstTools;
const zod_1 = require("zod");
const apiClient_js_1 = require("../services/apiClient.js");
const constants_js_1 = require("../constants.js");
function registerGstTools(server) {
    // ── Tool 1: Verify GSTIN ──────────────────────────────────────────────────
    server.registerTool("gst_verify_gstin", {
...
This resource is shared with Stage 3 vendor tools — no re-verification needed.`,
        inputSchema: zod_1.z.object({
            gstin: zod_1.z.string().length(15).describe("15-character GSTIN"),
            entity_name: zod_1.z.string().describe("Declared entity/applicant name to validate against GST records"),
            context: zod_1.z.enum(["borrower", "vendor"]).default("borrower")
        ...
        const result = await (0, apiClient_js_1.callApi)(mockKey, async () => {
            ...
            const resp = await axios.get(`${constants_js_1.API_ENDPOINTS.GST_VERIFY}/${gstin}`, { headers: { "X-API-Key": constants_js_1.API_KEYS.GST_API_KEY } });
            return resp.data;
        });
        const reasons = [];
        if (result.status !== "active")
            reasons.push(`GST registration is ${result.status}`);
        if (result.shell_entity_risk_score > constants_js_1.THRESHOLDS.MAX_SHELL_ENTITY_SCORE) {
            reasons.push(`High shell entity risk (${result.shell_entity_risk_score}/100)`);
            reasons.push(...result.shell_entity_signals);
        }
        if (result.months_since_last_filing > constants_js_1.THRESHOLDS.MAX_GST_MONTHS_UNFILED) {
    ...
    server.registerTool("gst_get_filing_history", {
...
Use in Stage 7 working capital monitoring to verify monthly business activity.`,
        inputSchema: zod_1.z.object({
            gstin: zod_1.z.string().length(15).describe("15-character GSTIN"),
            months: zod_1.z.number().int().min(1).max(36).default(12).describe("Months of history to retrieve"),
        }),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    }, async ({ gstin, months }) => {
        ...
        const output = {
            ...
            retrieved_at: (0, apiClient_js_1.now)(),
        };
        return {
            content: [{ type: "text", text: JSON.stringify(output, null, 2) }],
            structuredContent: output,
...
//# sourceMappingURL=gstTools.js.map