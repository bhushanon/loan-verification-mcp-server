...
function registerOrchestrationTools(server) {
    const orchestrateStage2VerificationSchema = zod_1.z.object({
        case_id: zod_1.z.string().describe("Unique loan application case ID"),
        applicant_pan: zod_1.z.string().length(10).describe("Applicant's PAN number"),
        applicant_name: zod_1.z.string().describe("Full applicant name"),
        loan_type: zod_1.z.enum(["clinic_setup", "equipment", "working_capital", "expansion"]),
        uploaded_docs: zod_1.z.array(zod_1.z.string()).describe("List of uploaded document type keys"),
        gstin: zod_1.z.string().optional().describe("GSTIN if business entity is involved"),
        dental_registration_number: zod_1.z.string().optional().describe("DCI registration number"),
    });
    // ── Tool 1: Run Full Stage 2 Verification Pipeline ─────────────────────
    server.registerTool("orchestrate_stage2_verification", {
...
Use this as your workflow map. Execute each step, check verdicts, stop on BLOCK.`,
    ...
    }, async ({ case_id, applicant_pan, applicant_name, loan_type, uploaded_docs, gstin, dental_registration_number }) => {
        const sequence = [
            {
                step: 1,
...
STOP CONDITION: Any tool returning risk_verdict=BLOCK halts the pipeline.`,
            created_at: (0, apiClient_js_1.now)(),
        };
        return {
    ...
    const orchestrateGenerateVerificationSummarySchema = zod_1.z.object({
        case_id: zod_1.z.string(),
        pan_verdict: zod_1.z.enum(["pass", "fail", "pending"]).default("pending"),
        aadhaar_verdict: zod_1.z.enum(["pass", "fail", "pending"]).default("pending"),
        credential_verdict: zod_1.z.enum(["pass", "fail", "pending"]).default("pending"),
        fraud_verdict: zod_1.z.enum(["pass", "fail", "pending"]).default("pending"),
        gst_verdict: zod_1.z.enum(["pass", "fail", "pending", "not_applicable"]).default("not_applicable"),
        blocking_issues: zod_1.z.array(zod_1.z.string()).default([]).describe("List of BLOCK reasons"),
        warnings: zod_1.z.array(zod_1.z.string()).default([]).describe("List of FLAG/warning reasons"),
    });
    // ── Tool 2: Generate Verification Summary ─────────────────────────────────
    server.registerTool("orchestrate_generate_verification_summary", {
...
(Camunda/Temporal) to advance the loan case to Stage 3 or route to exception handling.`,
    ...
    }, async ({ case_id, pan_verdict, aadhaar_verdict, credential_verdict, fraud_verdict, gst_verdict, blocking_issues, warnings }) => {
        const verdicts = { pan: pan_verdict, aadhaar: aadhaar_verdict, gst: gst_verdict, credential: credential_verdict, fraud_check: fraud_verdict, documents: "pass" };
        ...
        if (blocking_issues.length > 0)
            recommended.push("ESCALATE to fraud team immediately");
        if (warnings.length > 0)
            recommended.push("REVIEW warnings with credit officer before Stage 3");
        if (proceed)
            recommended.push("ADVANCE to Stage 3: Project & Machinery Validation");
        if (pending > 0)
            recommended.push(`COLLECT pending information: ${Object.entries(verdicts).filter(([, v]) => v === "pending").map(([k]) => k).join(", ")}`);
        const summary = {
            case_id,
            overall_risk_score: overallScore,
            risk_level: (0, apiClient_js_1.riskLevel)(overallScore),
            verification_statuses: verdicts,
            blocking_issues,
            warnings,
            proceed_to_next_stage: proceed,
            recommended_actions: recommended,
            summary_generated_at: (0, apiClient_js_1.now)(),
        };
        return {
