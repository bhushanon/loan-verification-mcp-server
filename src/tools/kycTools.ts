...
import { z } from "zod";
import { callApi, nameMatchScore, now } from "../services/apiClient.js";
import { API_ENDPOINTS, API_KEYS, THRESHOLDS } from "../constants.js";
import type { PANValidationResult, AadhaarVerificationResult } from "../types/index.js";

export function registerKycTools(server: McpServer): void {
...
Watchlist hits → always BLOCK and escalate to fraud team.`,
      inputSchema: z.object({
        pan: z.string().length(10).toUpperCase().describe("10-character PAN number"),
        applicant_name: z.string().min(2).describe("Full name as given in application"),
        check_dedup: z.boolean().default(true).describe("Check for duplicate PAN in bank records"),
      }),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    },
    async ({ pan, applicant_name, check_dedup }) => {
      const mockKey = `pan_verify_${pan}`;
      const result = await callApi<PANValidationResult>(
        mockKey,
        async () => {
          const axios = (await import("axios")).default;
          const resp = await axios.post(
            API_ENDPOINTS.PAN_VERIFY,
            { pan, check_dedup },
            { headers: { "X-API-Key": API_KEYS.PAN_API_KEY } }
          );
          return resp.data;
        }
      );
      ...
      const matchScore = result.name_match_score ?? nameMatchScore(applicant_name, result.name_on_pan);
      const reasons: string[] = [];

      if (!result.valid_format) reasons.push("PAN format invalid");
      if (matchScore < THRESHOLDS.MIN_NAME_MATCH_SCORE) {
...
Note: Aadhaar is never stored in full. Only last 4 digits used for reference.`,
      inputSchema: z.object({
        aadhaar_last4: z.string().length(4).describe("Last 4 digits of Aadhaar number"),
        name_on_application: z.string().describe("Name as given in the loan application"),
        address_state: z.string().describe("State declared in the application"),
        face_image_reference: z.string().optional().describe("Optional: face image reference for video KYC"),
      }),
    ...
    async ({ aadhaar_last4, name_on_application, address_state, face_image_reference }) => {
      ...
      const result = await callApi<AadhaarVerificationResult>(
        mockKey,
        async () => {
          const axios = (await import("axios")).default;
          const resp = await axios.post(
            API_ENDPOINTS.AADHAAR_VERIFY,
            { aadhaar_last4, face_image_reference },
            { headers: { "X-API-Key": API_KEYS.AADHAAR_API_KEY } }
          );
          return resp.data;
        }
      );
      ...
      const reasons: string[] = [];
      if (result.verification_status === "not_found") reasons.push("Aadhaar number not found in UIDAI database");
      if (!result.name_match) reasons.push(`Name mismatch (score: ${result.name_match_score})`);
      if (!result.address_consistency) reasons.push("Address does not match Aadhaar records");
      if (result.address_risk_signals.length > 0) reasons.push(`Address signals: ${result.address_risk_signals.join(", ")}`);
      if (face_image_reference && result.face_match_score !== null && result.face_match_score < THRESHOLDS.MIN_FACE_MATCH_SCORE) {
        reasons.push(`Face match failed (score: ${result.face_match_score}/100, minimum: ${THRESHOLDS.MIN_FACE_MATCH_SCORE})`);
      }
